...
class Dictionary:
    ...
    def add_new_entry(self):
        ...
        existing_entry = next(
            (entry for entry in self.entries if entry.word == word), None)
        ...
        while True:
            while True:
                ...
                if not word_type:
                    print(
                        "Word type cannot be blank. Please enter a valid word type."
                    )
                    continue
                break
            ...
            while True:
                ...
                if not meaning:
                    print(
                        "Meaning cannot be blank. Please enter a valid meaning."
                    )
                    continue
                break
            ...
            while True:
                ...
                if not example:
                    print(
                        "Example cannot be blank. Please enter a valid example."
                    )
                    continue
                break
            ...
            more = input(
                "Do you want to add another meaning for this word? (yes/no): "
            ).lower().strip()
            if more != 'yes':
                break
    ...
    def delete_entry(self, word):
        ...
        for line in lines:
            if line.strip().startswith("Word:") and line.strip().split(
                    ":")[1].strip() == word:
                word_found = True
                skip = True
                ...
                if entry.word in merged_entries:
                    ...
                    for word_type, linked_list in entry.meanings.items():
                        if word_type in existing_entry.meanings:
                            existing_meanings = existing_entry.meanings[
                                word_type].to_list()
                            new_meanings = linked_list.to_list()
                            for new_meaning in new_meanings:
                                if not any(m.meaning == new_meaning.meaning
                                           and m.example == new_meaning.example
                                           for m in existing_meanings):
                                    existing_entry.meanings[word_type].append(
                                        new_meaning)
                        else:
    ...
    #     # Preserve existing entries in the temporary dictionary
    #     existing_temp_entries = {entry.word: entry for entry in self.entries}

    #     for entry in existing_entries:
    #         if entry.word not in existing_temp_entries:
    #             # Add entry from file to the temporary dictionary
    #             self.entries.append(entry)
    ...
    def load_from_file(self):
        ...
        # Merge entries from file into temporary dictionary
        existing_temp_entries = {entry.word: entry for entry in self.entries}

        for entry in existing_entries:
            if entry.word in existing_temp_entries:
                # Update meanings for existing entry in temporary dictionary
                temp_entry = existing_temp_entries[entry.word]
                for word_type, linked_list in entry.meanings.items():
                    if word_type in temp_entry.meanings:
                        existing_meanings = temp_entry.meanings[
                            word_type].to_list()
                        new_meanings = linked_list.to_list()
                        for new_meaning in new_meanings:
                            if not any(m.meaning == new_meaning.meaning
                                       and m.example == new_meaning.example
                                       for m in existing_meanings):
                                temp_entry.meanings[word_type].append(
                                    new_meaning)
                    else:
                        temp_entry.meanings[word_type] = linked_list
            else:
                # Add entry from file to temporary dictionary
                self.entries.append(entry)
            ...
            while True:
                ...
                if choice2 == '1':
                    ...
                    while True:
                        ...
                        if not word:
                            print(
                                "Word cannot be blank. Please enter a valid word."
                            )
                            continue
                        break
                    ...
                    if dictionary.remove_entry(word):
                        print(
                            f"'{word}' has been successfully removed from the dictionary."
                        )
                        break
                    else:
                    ...
                    while True:
                        word = input(
                            "Enter the word of the entry to be deleted from the dictionary from the file: "
                        ).strip()
                        if not word:
                            print(
                                "Word cannot be blank. Please enter a valid word."
                    ...
                    if dictionary.delete_entry(word):
                        print(
                            f"'{word}' has been successfully removed from the dictionary in file."
                        )
                        break
                    else:
                        print(f"'{word}' does not exist in the dictionary.")
