...
class Dictionary:
    ...
    def add_new_entry(self):
        ...
        existing_entry = self.bst.find(word)

        if existing_entry is None:
            existing_entry = DictionaryEntry(word)
            self.bst.insert(existing_entry)
    ...
    def save_to_file(self):
        ...
        # Merge existing entries with the current BST entries
        merged_entries = {entry.word: entry for entry in existing_entries}
        new_entries = self.bst.in_order_traversal()
    ...
    def load_from_file(self):
        ...
        # Merge existing entries with the current BST dictionary
        for entry in existing_entries:
            existing_entry = self.bst.find(entry.word)
            if existing_entry:
                # Merge meanings if the entry already exists in the BST dictionary
                for word_type, linked_list in entry.meanings.items():
                    if word_type in existing_entry.meanings:
                        existing_meanings = existing_entry.meanings[word_type].to_list()
                        new_meanings = linked_list.to_list()
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            if not dictionary.bst.root:
                print("No entries in the temporary dictionary to save.")
            else:
